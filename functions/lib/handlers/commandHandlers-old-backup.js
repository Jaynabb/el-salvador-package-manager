...
async function handleExportCommand(senderPhone, args, options) {
    // Validate arguments
    if (args.length === 0) {
        await (0, twilioService_1.sendWhatsAppMessage)(senderPhone, "❌ Missing doc ID\n\nUsage: /export <doc-id>\nExample: /export doc-abc123");
        return;
    }
    const docId = args[0];
    // Find doc
    ...
    if (!docDoc.exists) {
        await (0, twilioService_1.sendWhatsAppMessage)(senderPhone, `❌ Doc not found: ${docId}\n\nMake sure the doc ID is correct.`);
        return;
    }
    const doc = Object.assign({ id: docDoc.id }, docDoc.data());
    ...
    if (doc.status !== "ready" && doc.status !== "active") {
        await (0, twilioService_1.sendWhatsAppMessage)(senderPhone, `⚠️ Doc not ready for export\n\nDoc: ${doc.customerName || docId}\nStatus: ${doc.status}\n\nPlease process the doc in the web app first.`);
        return;
    }
    try {
        // Send processing message
        await (0, twilioService_1.sendWhatsAppMessage)(senderPhone, `⏳ Processing export for doc: ${doc.customerName || docId}\n\nThis may take a moment...`);
        // Get all orders for this doc
        const ordersSnapshot = await getDb().collection("orders")
            .where("docId", "==", docId)
            .get();
        if (ordersSnapshot.empty) {
            await (0, twilioService_1.sendWhatsAppMessage)(senderPhone, `❌ No orders found for doc: ${docId}`);
            return;
        }
        ...
        await (0, twilioService_1.sendWhatsAppMessage)(senderPhone, `❌ Failed to export doc\n\nError: ${error}\n\nPlease try again or contact support.`);
    }
}
/**
...
async function handleStatusCommand(senderPhone, args) {
    ...
    const message = [
        ...
        `Status: ${doc.status}`,
        `Orders: ${orderCount}`,
        `Created: ${new Date(((_b = (_a = doc.createdAt) === null || _a === void 0 ? void 0 : _a.toDate) === null || _b === void 0 ? void 0 : _b.call(_a)) || doc.createdAt).toLocaleDateString()}`,
    ];
    if (doc.totalWeight) {
    ...
    await (0, twilioService_1.sendWhatsAppMessage)(senderPhone, message.join("\n"));
}
/**
 * Handle /help command
 */
async function handleHelpCommand(senderPhone, args) {
    ...
    const helpText = (0, commandParser_1.getCommandHelp)(specificCommand);
    await (0, twilioService_1.sendWhatsAppMessage)(senderPhone, helpText);
}
//# sourceMappingURL=commandHandlers-old-backup.js.map