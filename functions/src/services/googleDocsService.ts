 ...
 */
export async function createBatchDocument(packageData: any): Promise<string> {
  try {
 ...
 */
function buildBatchDocumentRequests(packageData: any): any[] {
  const requests: any[] = [];
  let index = 1;
  ...
  if (packageData.trackingNumbers && packageData.trackingNumbers.length > 0) {
    ...
    packageData.trackingNumbers.forEach((tracking: string, i: number) => {
  ...
  packageData.items.forEach((item: any, i: number) => {
  ...
  addText(
    `GRAND TOTAL: $${(packageData.totalValue + packageData.totalFees).toFixed(2)} USD\n`,
    true,
    12
  );
  ...
  addText(
    `Declared Value: $${packageData.customsDeclaration.declaredValue.toFixed(2)} USD\n`
  );
  ...
  if (packageData.customsDeclaration.certificateOfOrigin !== undefined) {
    addText(
      `Certificate of Origin: ${packageData.customsDeclaration.certificateOfOrigin ? "Yes" : "No"}\n`
    );
  }
  addText("\n");
